...
_MAIL_DEDUPE_WINDOW_SECONDS = 600
...
class DeviceAuthRelay:
    ...
    async def maybe_handle(self, event: TelegramEvent) -> bool:
        ...
        if outcome.status == "FORWARDED":
            ...
            self.alerts.info("DEVICE_AUTH_RELAY_DEDUPED", "device auth email forwarding skipped as duplicate", payload)
    ...
    def _process_trigger(self, event: TelegramEvent) -> RelayOutcome:
        ...
        if self._is_recent_duplicate(matched, trigger_at=event.date):
            return RelayOutcome(
                status="DEDUPED",
                detail=f"duplicate mail skipped within {_MAIL_DEDUPE_WINDOW_SECONDS}s window",
                subject=matched.subject,
                mail_date=matched.mail_date.isoformat(),
                matched_server=matched.matched_server,
            )
        self._forward_mail(matched)
        self._mark_forwarded(matched, trigger_at=event.date)
        return RelayOutcome(
            status="FORWARDED",
            detail="forwarded",
            subject=matched.subject,
    ...
    def _is_recent_duplicate(self, mail: _MatchedMail, *, trigger_at: datetime) -> bool:
        key = self._dedupe_key(mail)
        raw_value = self.store.get_system_flag(key)
        if raw_value is None:
            return False
        try:
            last_ts = float(raw_value)
        except Exception:  # noqa: BLE001
            return False
        trigger_ts = trigger_at.timestamp()
        return trigger_ts - last_ts < _MAIL_DEDUPE_WINDOW_SECONDS

    def _mark_forwarded(self, mail: _MatchedMail, *, trigger_at: datetime) -> None:
        self.store.set_system_flag(self._dedupe_key(mail), str(trigger_at.timestamp()))

    @staticmethod
    def _dedupe_key(mail: _MatchedMail) -> str:
        fingerprint = hashlib.sha256(
            f"{mail.subject}|{mail.mail_date.isoformat()}|{mail.matched_server}".encode("utf-8", errors="ignore")
        ).hexdigest()
        return f"device_auth_relay::mail::{fingerprint}"
