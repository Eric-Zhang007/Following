...
def _config(max_notional: float = 200.0) -> AppConfig:
...
def test_position_sizing_uses_account_risk_per_trade_and_notional_cap() -> None:
    manager = RiskManager(_config(max_notional=200))
    signal = EntrySignal(
        kind=ParsedKind.ENTRY_SIGNAL,
        raw_text="#BTC/USDT 进场: 限价100-100",
        symbol="BTCUSDT",
        quote="USDT",
        side=Side.LONG,
        leverage=10,
    ...
    assert decision.stop_distance_ratio == 0.006
    assert round(float(decision.quantity or 0), 6) == 2.0
    assert round(float(decision.notional or 0), 6) == 200.0
